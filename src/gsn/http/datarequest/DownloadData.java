...
//    }
            ...
            while (iter.hasNext()) {
                ...
                while (de.hasMoreElements()) {
                    if (ot == AllowedOutputType.csv) {
                      ...
                      else if (ot == AllowedOutputType.jpeg) {
                        de.MakeReport();
                        StreamElement element = de.nextElement();
                        formatJPEGElement(os, element);
                    }
                    firstLine = false;
                }
                if (ot == AllowedOutputType.xml)
    ...
    private boolean areAllElementsPictures(StreamElement element) {
        Serializable[] values = element.getData();
        boolean areAllPictures = false;

        for(Serializable value : values){
            areAllPictures &= (value instanceof byte[]);
        }
        return areAllPictures;
    }
    ...
    private void formatJPEGElement(OutputStream respond, StreamElement se)
    {
        Serializable[] elements = se.getData();

        for(Serializable element : elements){
            if(element instanceof byte[]){
                byte[] myBytes = (byte[]) element;
                try {
                    respond.write(myBytes);
                } catch (IOException e) {
                    logger.error("Problem with outputstream: " + e);
                }
            }
        }
    }
