...
public class RabbitBrojiloWrapper extends AbstractWrapper {
	...
	private final transient Logger logger = Logger.getLogger(RabbitBrojiloWrapper.class);
	...
	public void run(){
		while(isActive()){
			...
			StreamElement streamElement = new StreamElement(FIELD_NAMES, new Byte[] {DataTypes.DOUBLE }, new Serializable[] {getTemperature()}, System.currentTimeMillis());
	...
	private Serializable getTemperature() {
		...
		try {
			...
			if(handler.toString() != null && handler.toString() != "")
				temperature = Double.parseDouble(handler.toString());
		} catch (Exception e) {
